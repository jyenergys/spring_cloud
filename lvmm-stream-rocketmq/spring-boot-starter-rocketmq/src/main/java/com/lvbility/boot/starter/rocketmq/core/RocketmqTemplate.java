package com.lvbility.boot.starter.rocketmq.core;
...
public class RocketmqTemplate extends AbstractMessageSendingTemplate<String> implements InitializingBean, DisposableBean {
    ...
    public SendResult syncSend(org.apache.rocketmq.common.message.Message rocketMsg){
    	try {
			SendResult sendResult = producer.send(rocketMsg, producer.getSendMsgTimeout());
			return sendResult;
		} catch (Exception e) {
            log.info("syncSend failed. message:{} ", rocketMsg);
            throw new MessagingException(e.getMessage(), e);
        }
    }
    ...
    public void sendOneWay(org.apache.rocketmq.common.message.Message rocketMsg){
    	 try {
			producer.sendOneway(rocketMsg);
		} catch (Exception e) {
			log.info("sendOneWay failed.  message:{} ", rocketMsg);
            throw new MessagingException(e.getMessage(), e);
		}
    }
     ...
     * @return
     */
    public String[] convertToToicAndTags(String destination){
        ...
        return new String[]{topic, tags};
    }
    
    /**
     * spring msg转化为rocketmq msg
     * @param topic
     * @param tags
     * @param message
     * @return
     */
    public org.apache.rocketmq.common.message.Message convertToRocketMsg(String topic, String tags, Message<?> message) {
        ...
        if (Objects.nonNull(headers) && !headers.isEmpty()) {
            ...
            Object delayTimeLevelObj = headers.get(MessageConst.PROPERTY_DELAY_TIME_LEVEL);
            if(delayTimeLevelObj != null){
	            int delayTimeLevel = Integer.valueOf(delayTimeLevelObj + "");
				if(delayTimeLevel  > 0){
    ...
    public org.apache.rocketmq.common.message.Message convertToRocketMsg(String destination, Message<?> message) {
    	...
    	return convertToRocketMsg(topic, tags, message);
    }
